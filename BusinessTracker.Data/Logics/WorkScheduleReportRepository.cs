...
/// <summary>
/// Реализация репозитория отчёта "График работы".
/// Загружает транзакции из БД, преобразует в доменные модели,
/// затем строит отчёт через <see cref="WorkScheduleReportBuilder"/>.
/// </summary>
public class WorkScheduleReportRepository(BusinessTrackerContext context) : IWorkScheduleReportRepository
{
    public async Task<IEnumerable<WorkScheduleReportRowDto>> GetReport(
        Guid organizationId,
        DateTimeOffset from,
        DateTimeOffset to,
        CancellationToken cancellationToken)
    {
        ...
        return WorkScheduleReportBuilder.Build(domainTransactions);
    }
}