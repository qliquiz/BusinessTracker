...
(
    ...
    "LoadOptions"    JSONB,
    CONSTRAINT "FK_Branches_Organizations" FOREIGN KEY ("OrganizationId") REFERENCES "Organizations" ("Id") ON DELETE CASCADE
);
...
SELECT CASE "Id"::text
           WHEN 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11' THEN 'f0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11'::uuid
...
ALTER TABLE "Organizations"
    DROP COLUMN IF EXISTS "LoadOptions";
...
$$
    ...
    BEGIN
        ...
        RAISE NOTICE 'Organizations  : %  (unchanged)', v_organizations;
        RAISE NOTICE 'Branches       : %  (new, = Organizations)', v_branches;
        RAISE NOTICE 'Categories     : %  (unchanged)', v_categories;
        RAISE NOTICE 'Nomenclatures  : %  (unchanged)', v_nomenclatures;
        RAISE NOTICE 'Employees      : %  (unchanged)', v_employees;
        RAISE NOTICE 'Transactions   : %  (unchanged)', v_transactions;
        RAISE NOTICE 'JournalRows    : %  (unchanged)', v_journal_rows;
        RAISE NOTICE '=====================================';

        -- Проверяем инвариант: кол-во филиалов = кол-ву организаций
        IF v_branches <> v_organizations THEN
